...
struct BallDropButton: View {
    ...
    var body: some View {
        ...
        Picker("Draw", selection: $userSettings.drawMethod) {
            ...
            Text("EuroMillions").tag(UserSettings.numberType.EuroMillions) // ✅ Added option
        }
        .padding(.vertical, 3)
        .pickerStyle(.segmented)
        .background(
            ZStack {
                // **Dark semi-transparent background for a premium feel**
                Color.black.opacity(0.5)
                // **Golden Stroke with Opacity & Blur**
                VStack {
                    // **Top Stroke**
                    Rectangle()
                        .fill(
                        ...
                        .frame(height: 1.3) // Slightly thicker for a luxurious effect
                        .blur(radius: 0.5) // Adds a soft glow effect
        ...
        .onChange(of: userSettings.drawMethod) { newValue in
            ...
            } else if newValue == .EuroMillions { // ✅ Open EuroMillionsView when selected
                DispatchQueue.main.async {
        ...
        // ✅ Open EuroMillionsView in Full-Screen
